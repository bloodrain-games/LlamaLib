...
class UNDREAMAI_API LLMServiceBuilder
{
  ...
  public:
    ...
    LLMService *build()
    {
        LLMService *service = new LLMService(model_path_, num_slots_, num_threads_, num_GPU_layers_, flash_attention_,
                                             context_size_, batch_size_, embedding_only_, lora_paths_);
        return service;
    }
};
